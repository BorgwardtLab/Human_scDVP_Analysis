...
def mouse_to_human(gene_list):
    ...
    map_df = findOrthologsMmHs(
        ...
        from_values = gene_list
        ).map()

    mouse_genes = map_df.external_gene_name.values
    human_genes = map_df.hgnc_symbol.values

    return {mouse: human for mouse, human in zip(mouse_genes, human_genes) if pd.notnull(human)}
...
def filter_samples_by_protein_count(df):
    ...
    return df.loc[:, (protein_counts >= lower_threshold) & (protein_counts <= upper_threshold)]